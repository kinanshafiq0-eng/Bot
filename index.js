...
function getCurrency(userId) {
  return db.currency[userId] || 0;
}
function addCurrency(userId, amount) {
  db.currency[userId] = (db.currency[userId] || 0) + amount;
  return db.currency[userId];
}
...
client.on('messageCreate', async (message) => {
  ...
  if (db.messageCounter[userId] % 30 === 0) {
    ...
    try {
      await message.author.send(`🌟 حصلت على 15 عملة! رصيدك: ${newBal}`);
    } catch (e) {}
  }
});
...
client.on('voiceStateUpdate', async (oldState, newState) => {
  ...
  if (!oldState.channelId && newState.channelId) {
    const interval = setInterval(() => {
      ...
      try {
        newState.member.send(`🔊 +1 عملة (فويس). رصيدك: ${newBal}`);
      } catch (e) {}
    }, 60000);
    db.voiceTimers[key] = interval;
  } else if (oldState.channelId && !newState.channelId) {
    if (db.voiceTimers[key]) {
      clearInterval(db.voiceTimers[key]);
      delete db.voiceTimers[key];
    }
  }
});
...
client.on('messageCreate', async (message) => {
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        { name: '🛡️ الإدارة', value: '`حظر` `طرد` `كتم` `فك_كتم` `تحذير` `مسح` `قفل` `فتح`', inline: false },
        {
          name: '⚙️ الإعدادات',
          value:
            '`تعيين سجلات #قناة` `تعيين ترحيب #قناة` `تعيين رسالة_ترحيب نص` `تعيين صورة_ترحيب رابط` `تعيين دور_كتم @دور` `تعيين دور_دخول @دور` `تعيين حد_كتم عدد` `تعيين حد_طرد عدد`',
          inline: false,
        },
  ...
  if (cmd === 'حظر') {
    ...
    await message.reply(
      `⚠️ تم تحذير ${member.user.tag} بسبب: ${reason} (إجمالي التحذيرات: ${count})`
    );
    try {
      await member.send(`⚠️ تم تحذيرك في ${message.guild.name} بسبب: ${reason}`);
    } catch (e) {}
    await logToChannel(guildId, {
      title: '⚠️ تحذير',
...
client.on('interactionCreate', async (interaction) => {
    ...
    // زر تغيير الاسم
      ...
      if (last && Date.now() - last < 5 * 60 * 60 * 1000) {
        ...
        return interaction.reply({
          content: `⏳ يمكنك تغيير اسمك بعد ${remaining} ساعة.`,
          ephemeral: true,
        });
      }
  ...
  if (interaction.isModalSubmit() && interaction.customId === 'name_change_modal') {
    ...
    if (newName.length < 2 || newName.length > 32) {
      return interaction.reply({
        content: '⚠️ الاسم يجب أن يكون بين 2 و 32 حرفاً.',
        ephemeral: true,
      });
    }
    try {
