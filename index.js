...
// دالة رسم العجلة (تم تعديلها لتدوير العجلة نحو الفائز)
async function generateRouletteImage(players, winnerIndex) {
    ...
    // حساب زاوية الدوران عشان السهم (اللي زاويته 0 على اليمين) يأشر على منتصف قطعة الفائز
    const offsetAngle = - (winnerIndex * sliceAngle + sliceAngle / 2);

    for (let i = 0; i < players.length; i++) {
        // إضافة زاوية الدوران لكل قطعة
        ...
        // إضافة الأسماء (تغيير حجم الخط حسب العدد عشان ما يتداخلوا)
        ctx.save();
...
client.on('messageCreate', async message => {
    ...
    if (message.content === '!roulette') {
        ...
        const MAX_PLAYERS = 20; // الحد الأقصى للاعبين
        
        // حساب الوقت بعد 30 ثانية بصيغة ديسكورد الذكية
        ...
        // المؤقت 30 ثانية (30000 ميلي ثانية)
        ...
        collector.on('collect', async interaction => {
            ...
            // تحديث قائمة الأسماء في الإيمبد
        ...
        collector.on('end', async (collected, reason) => {
            // إذا انتهى الوقت أو تم ضغط زر "بدء الآن"
