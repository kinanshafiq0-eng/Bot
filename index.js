...
const db = {
  config: {}, nameCooldown: {}, memberCount: {}, ticketSettings: {},
  warns: {}, autoLine: {}, autoReplies: {}, users: {}, levelRoles: {},
  controllers: {},
};
...
function getGuildConfig(guildId) {
  if (!db.config[guildId]) {
    db.config[guildId] = {
      ...
      welcomeImage: null, muteRole: null, joinRole: null,
      ticketPanelImage: null, rolesImage: null,
      bannerImage: null, generalImage: null, levelChannelId: null,
    };
  }
...
client.on('messageDelete', async (message) => {
  ...
  await logToChannel(message.guild.id, {
    title: '🗑️ حذف رسالة', color: 0xff0000,
...
client.on('messageUpdate', async (oldMessage, newMessage) => {
  ...
  await logToChannel(oldMessage.guild.id, {
    title: '✏️ تعديل رسالة', color: 0xffaa00,
...
client.on('messageCreate', async (message) => {
  ...
  if (cmd === 'مساعدة') {
    const embed = new EmbedBuilder()
      ...
      .setColor(0x2b2d31)
      .addFields(
        { name: '👑 نظام التحكم', value: '`متحكم @شخص` (للمالك)  `الغاء_متحكم @شخص` (للمالك)  `قائمة_المتحكمين`', inline: false },
  ...
  if (cmd === 'اختبار_لوق') {
    ...
    await logToChannel(guildId, {
      title: '🧪 اختبار اللوق', color: 0x00ff00,
  ...
  if (cmd === 'تعيين' && args[0]?.toLowerCase() === 'ترحيب') {
    ...
    if (!channel) {
      ...
      await logToChannel(guildId, { title: '⚙️ إعدادات', color: 0x2b2d31, description: `**${message.author}** ألغى قناة الترحيب.`, footer: 'الإعدادات' });
      return message.reply('✅ تم إلغاء تحديد قناة الترحيب.');
    }
    updateGuildConfig(guildId, { welcomeChannel: channel.id });
    await logToChannel(guildId, { title: '⚙️ إعدادات', color: 0x2b2d31, description: `**${message.author}** عيّن قناة الترحيب إلى ${channel}.`, footer: 'الإعدادات' });
  ...
  if (cmd === 'تعيين' && args[0]?.toLowerCase() === 'صورة_ترحيب') {
    ...
    if (!url) {
      ...
      await logToChannel(guildId, { title: '⚙️ إعدادات', color: 0x2b2d31, description: `**${message.author}** ألغى صورة الترحيب.`, footer: 'الإعدادات' });
      return message.reply('✅ تم إلغاء صورة الترحيب.');
    }
    updateGuildConfig(guildId, { welcomeImage: url });
    await logToChannel(guildId, { title: '⚙️ إعدادات', color: 0x2b2d31, description: `**${message.author}** عيّن صورة الترحيب: ${url}`, footer: 'الإعدادات' });
  ...
  if (cmd === 'تعيين' && args[0]?.toLowerCase() === 'عنوان_ترحيب') {
    ...
    await logToChannel(guildId, { title: '⚙️ إعدادات', color: 0x2b2d31, description: `**${message.author}** غيّر عنوان الترحيب إلى: "${text}"`, footer: 'الإعدادات' });
  ...
  if (cmd === 'تعيين' && args[0]?.toLowerCase() === 'سجلات') {
    ...
    await logToChannel(guildId, { title: '📋 تم تعيين قناة اللوق', color: 0x2b2d31, description: `**${message.author}** عيّن قناة اللوق إلى ${channel}`, footer: 'الإعدادات' });
  ...
  if (cmd === 'تعيين' && args[0]?.toLowerCase() === 'روم_ليفل') {
    ...
    if (!channel) {
      ...
      await logToChannel(guildId, { title: '⚙️ إعدادات', color: 0x2b2d31, description: `**${message.author}** ألغى قناة الليفل.`, footer: 'الإعدادات' });
      return message.reply('✅ تم إلغاء تحديد قناة الليفل.');
    }
    updateGuildConfig(guildId, { levelChannelId: channel.id });
    await logToChannel(guildId, { title: '⚙️ إعدادات', color: 0x2b2d31, description: `**${message.author}** عيّن قناة الليفل إلى ${channel}.`, footer: 'الإعدادات' });
  ...
  if (cmd === 'حظر') {
    ...
    if (generalImage) embed.setImage(generalImage);
    await message.channel.send({ embeds: [embed] });
    await logToChannel(guildId, { title: '🔨 حظر', color: 0xff0000, description: `**المنفذ:** ${message.author}\n**المستهدف:** ${member.user.tag}\n**السبب:** ${reason}` });
    return;
  }
  ...
  if (cmd === 'طرد') {
    ...
    if (generalImage) embed.setImage(generalImage);
    await message.channel.send({ embeds: [embed] });
    await logToChannel(guildId, { title: '🚪 طرد', color: 0xff8800, description: `**المنفذ:** ${message.author}\n**المستهدف:** ${member.user.tag}\n**السبب:** ${reason}` });
    return;
  }
  ...
  if (cmd === 'كتم') {
    ...
    if (generalImage) embed.setImage(generalImage);
    await message.channel.send({ embeds: [embed] });
    await logToChannel(guildId, { title: '🔇 كتم', color: 0xffaa00, description: `**المنفذ:** ${message.author}\n**المستهدف:** ${member.user.tag}\n**السبب:** ${reason}` });
    return;
  }
  ...
  if (cmd === 'فك_كتم') {
    ...
    if (generalImage) embed.setImage(generalImage);
    await message.channel.send({ embeds: [embed] });
    await logToChannel(guildId, { title: '🔊 فك كتم', color: 0x00ff00, description: `**المنفذ:** ${message.author}\n**المستهدف:** ${member.user.tag}` });
    return;
  }
  ...
  if (cmd === 'تحذير') {
    ...
    if (generalImage) embed.setImage(generalImage);
    await message.channel.send({ embeds: [embed] });
    await logToChannel(guildId, { title: '⚠️ تحذير', color: 0xffdd00, description: `**المنفذ:** ${message.author}\n**المستهدف:** ${member.user.tag}\n**السبب:** ${reason}\n**عدد التحذيرات:** ${count}` });
    try {
  ...
  if (cmd === 'ابطال_تحذيرات') {
    ...
    if (generalImage) embed.setImage(generalImage);
    await message.channel.send({ embeds: [embed] });
    await logToChannel(guildId, { title: '✅ إبطال تحذيرات', color: 0x00ff00, description: `**المنفذ:** ${message.author}\n**المستهدف:** ${member.user.tag}` });
    return;
  }
  ...
  if (cmd === 'مسح') {
    ...
    await logToChannel(guildId, { title: '🗑️ مسح رسائل', color: 0x00ccff, description: `**المنفذ:** ${message.author}\n**القناة:** ${message.channel.name}\n**عدد الرسائل:** ${count}` });
    return;
  }
  ...
  if (cmd === 'قفل') {
    ...
    if (generalImage) embed.setImage(generalImage);
    await message.channel.send({ embeds: [embed] });
    await logToChannel(guildId, { title: '🔒 قفل قناة', color: 0xff0000, description: `**المنفذ:** ${message.author}\n**القناة:** ${message.channel.name}` });
    return;
  }
  ...
  if (cmd === 'فتح') {
    ...
    if (generalImage) embed.setImage(generalImage);
    await message.channel.send({ embeds: [embed] });
    await logToChannel(guildId, { title: '🔓 فتح قناة', color: 0x00ff00, description: `**المنفذ:** ${message.author}\n**القناة:** ${message.channel.name}` });
    return;
  }
  ...
  if (cmd === 'انشاء_قناة') {
    ...
    if (generalImage) embed.setImage(generalImage);
    await message.channel.send({ embeds: [embed] });
    await logToChannel(guildId, { title: '📁 إنشاء قناة', color: 0x00ccff, description: `**المنفذ:** ${message.author}\n**القناة:** ${channel.name}` });
    return;
  }
  ...
  if (cmd === 'حذف_قناة') {
    ...
    if (generalImage) embed.setImage(generalImage);
    await message.channel.send({ embeds: [embed] });
    await logToChannel(guildId, { title: '🗑️ حذف قناة', color: 0xff0000, description: `**المنفذ:** ${message.author}\n**القناة:** ${channelName}` });
    return;
  }
  ...
  if (cmd === 'طرد_صوتي') {
    ...
    if (generalImage) embed.setImage(generalImage);
    await message.channel.send({ embeds: [embed] });
    await logToChannel(guildId, { title: '🔊 طرد من الصوت', color: 0xff8800, description: `**المنفذ:** ${message.author}\n**المستهدف:** ${member.user.tag}` });
    return;
  }
  ...
  if (cmd === 'كتم_صوتي') {
    ...
    if (generalImage) embed.setImage(generalImage);
    await message.channel.send({ embeds: [embed] });
    await logToChannel(guildId, { title: '🔇 كتم صوتي', color: 0xffaa00, description: `**المنفذ:** ${message.author}\n**المستهدف:** ${member.user.tag}` });
    return;
  }
  ...
  if (cmd === 'فك_كتم_صوتي') {
    ...
    if (generalImage) embed.setImage(generalImage);
    await message.channel.send({ embeds: [embed] });
    await logToChannel(guildId, { title: '🔊 فك كتم صوتي', color: 0x00ff00, description: `**المنفذ:** ${message.author}\n**المستهدف:** ${member.user.tag}` });
    return;
  }
  ...
  if (cmd === 'تعيين' && args[0]?.toLowerCase() === 'تذكرة') {
    ...
    const settings = getTicketSettings(guildId);
    if (!sub) {
      const embed = new EmbedBuilder().setTitle('⚙️ إدارة التذاكر').setColor(0x2b2d31)
        .addFields(
    ...
    if (sub === 'حذف') {
      ...
      saveTicketSettings(guildId, settings);
      await logToChannel(guildId, { title: '🗑️ حذف قسم تذكرة', color: 0xff0000, description: `**${message.author}** حذف قسم **${sectionName}**` });
    ...
    if (sub === 'نص') {
      ...
      settings.text = value;
      saveTicketSettings(guildId, settings);
      await logToChannel(guildId, { title: '📝 تغيير نص التذاكر', color: 0x2b2d31, description: `**${message.author}** غيّر نص التذاكر إلى:\n${value}` });
    ...
    if (sub === 'صورة') {
      ...
      saveTicketSettings(guildId, settings);
      await logToChannel(guildId, { title: '🖼️ تغيير صورة التذاكر', color: 0x2b2d31, description: `**${message.author}** غيّر صورة التذاكر إلى: ${value}` });
  ...
  if (cmd === 'تعيين' && args[0]?.toLowerCase() === 'اوتر_لاين') {
    ...
    await logToChannel(guildId, { title: '🤖 تعيين أوتو لاين', color: 0x2b2d31, description: `**${message.author}** عيّن الأوتو لاين في ${channel}:\n${text}` });
    const embed = new EmbedBuilder().setTitle('✅ تم تعيين الأوتو لاين').setColor(0x2b2d31).setDescription(`**الروم:** ${channel}\n**النص:** ${text}`).setFooter({ text: 'تم التفعيل تلقائياً.' });
    if (generalImage) embed.setImage(generalImage);
    await message.channel.send({ embeds: [embed] });
    return;
  }
  ...
  if (cmd === 'تعيين' && args[0]?.toLowerCase() === 'صورة_اوترلاين') {
    ...
    await logToChannel(guildId, { title: '🖼️ تعيين صورة أوتو لاين', color: 0x2b2d31, description: `**${message.author}** عيّن صورة الأوتو لاين: ${url}` });
    const embed = new EmbedBuilder().setTitle('✅ تم تعيين صورة الأوتو لاين').setColor(0x2b2d31).setDescription(`[رابط الصورة](${url})`).setImage(url);
    if (generalImage) embed.setThumbnail(generalImage);
    await message.channel.send({ embeds: [embed] });
    return;
  }
  ...
  if (cmd === 'تعيين' && args[0]?.toLowerCase() === 'تفعيل_اوترلاين') {
    ...
    const embed = new EmbedBuilder().setTitle('✅ تم تفعيل الأوتو لاين').setColor(0x2b2d31).setDescription('تم تشغيل النظام. سيرد البوت تلقائياً في الروم المحدد.');
    if (generalImage) embed.setImage(generalImage);
    await message.channel.send({ embeds: [embed] });
    return;
  }
  ...
  if (cmd === 'تعيين' && args[0]?.toLowerCase() === 'تعطيل_اوترلاين') {
    ...
    const embed = new EmbedBuilder().setTitle('⏹️ تم تعطيل الأوتو لاين').setColor(0x2b2d31).setDescription('تم إيقاف النظام. لن يرد البوت تلقائياً حتى يتم تفعيله مرة أخرى.');
    if (generalImage) embed.setImage(generalImage);
    await message.channel.send({ embeds: [embed] });
    return;
  }
  ...
  if (cmd === 'رد_تلقائي') {
    ...
    const embed = new EmbedBuilder().setTitle(added ? '✅ تم إضافة رد تلقائي' : '🔄 تم تحديث رد تلقائي').setColor(0x2b2d31).setDescription(`**الكلمة:** ${keyword}\n**الرد:** ${reply}`).setFooter({ text: 'سيرد البوت تلقائياً عند كتابة هذه الكلمة.' });
    if (generalImage) embed.setImage(generalImage);
    await message.channel.send({ embeds: [embed] });
    return;
  }
  ...
  if (cmd === 'رد_تلقائي_صورة') {
    ...
    const embed = new EmbedBuilder().setTitle(added ? '✅ تم إضافة رد تلقائي مع صورة' : '🔄 تم تحديث رد تلقائي مع صورة').setColor(0x2b2d31).setDescription(`**الكلمة:** ${keyword}\n**الرد:** ${reply}`).setImage(image).setFooter({ text: 'سيرد البوت مع الصورة تلقائياً.' });
    if (generalImage) embed.setThumbnail(generalImage);
    await message.channel.send({ embeds: [embed] });
    return;
  }
  ...
  if (cmd === 'حذف_رد_تلقائي') {
    ...
    await logToChannel(guildId, { title: '🗑️ حذف رد تلقائي', color: 0xff0000, description: `**${message.author}** حذف الرد التلقائي للكلمة **${keyword}**` });
    const embed = new EmbedBuilder().setTitle('🗑️ تم حذف الرد التلقائي').setColor(0x2b2d31).setDescription(`تم حذف الرد التلقائي للكلمة: **${keyword}**`);
    if (generalImage) embed.setImage(generalImage);
    await message.channel.send({ embeds: [embed] });
    return;
  }
  ...
  if (cmd === 'عرض_الردود') {
    ...
    const embed = new EmbedBuilder().setTitle('💬 قائمة الردود التلقائية').setColor(0x2b2d31).setDescription(list).setFooter({ text: `عدد الردود: ${replies.length}` });
    if (generalImage) embed.setImage(generalImage);
    await message.channel.send({ embeds: [embed] });
    return;
  }
  ...
  if (cmd === 'تعيين') {
    ...
    if (sub === 'دور_دخول') {
      ...
      await logToChannel(guildId, { title: '⚙️ إعدادات', color: 0x2b2d31, description: `**${message.author}** عيّن دور الدخول إلى ${role.name}.`, footer: 'الإعدادات' });
      await message.reply(`✅ تم تعيين دور الدخول إلى ${role}`);
    } else if (sub === 'صورة_بانل') {
      if (!value) return message.reply('⚠️ أدخل رابط الصورة.');
      updateGuildConfig(guildId, { ticketPanelImage: value });
      await logToChannel(guildId, { title: '⚙️ إعدادات', color: 0x2b2d31, description: `**${message.author}** عيّن صورة البانل: ${value}`, footer: 'الإعدادات' });
      await message.reply(`✅ تم تعيين صورة البانل: ${value}`);
    } else if (sub === 'صورة_رتب') {
      if (!value) return message.reply('⚠️ أدخل رابط الصورة.');
      updateGuildConfig(guildId, { rolesImage: value });
      await logToChannel(guildId, { title: '⚙️ إعدادات', color: 0x2b2d31, description: `**${message.author}** عيّن صورة رتب الإشعارات: ${value}`, footer: 'الإعدادات' });
      await message.reply(`✅ تم تعيين صورة رتب الإشعارات: ${value}`);
    } else if (sub === 'صورة_بنر') {
      if (!value) return message.reply('⚠️ أدخل رابط الصورة.');
      updateGuildConfig(guildId, { bannerImage: value });
      await logToChannel(guildId, { title: '⚙️ إعدادات', color: 0x2b2d31, description: `**${message.author}** عيّن صورة البنر: ${value}`, footer: 'الإعدادات' });
      await message.reply(`✅ تم تعيين صورة البنر: ${value}`);
    } else if (sub === 'صورة_عامة') {
      if (!value) return message.reply('⚠️ أدخل رابط الصورة.');
      updateGuildConfig(guildId, { generalImage: value });
      await logToChannel(guildId, { title: '⚙️ إعدادات', color: 0x2b2d31, description: `**${message.author}** عيّن الصورة العامة: ${value}`, footer: 'الإعدادات' });
...
client.on('interactionCreate', async (interaction) => {
  ...
  if (interaction.isButton()) {
    ...
    if (interaction.customId === 'close_ticket') {
      ...
      await logToChannel(interaction.guild.id, { title: '🔒 إغلاق تذكرة', color: 0xff0000, description: `**المستخدم:** ${interaction.user}\n**القناة:** ${channel.name}\n**صاحب التذكرة:** ${ticketOwnerId ? `<@${ticketOwnerId}>` : 'غير معروف'}`, footer: 'نظام التذاكر' });
